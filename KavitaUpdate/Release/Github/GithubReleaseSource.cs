...
using KavitaUpdate.Database.Models;
using KavitaUpdate.Util;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Options;
using Octokit;

namespace KavitaUpdate.Release.Github
{
    public class GithubReleaseSource : ReleaseSourceBase
    {
        ...
        private static readonly Regex ReleaseFeaturesGroup = new Regex(@"\*\s+[0-9a-f]{40}\s+(New|Added):\s*(?<text>.*?)\r*$", RegexOptions.Compiled | RegexOptions.Multiline);
        ...
        private static readonly Regex ReleaseChangesGroup = new Regex(@"\*\s+[0-9a-f]{40}\s+Changed:\s*(?<text>.*?)\r*$", RegexOptions.Compiled | RegexOptions.Multiline);

        private const string RepoName = "UpdaterTest"; // TODO: Change from UpdaterTest to Kavita for final release
        private enum Branch
        {
            Master,
            Develop
        ...
        public GithubReleaseSource(DatabaseContext database, IOptions<Config> config)
        {
            ...
            _gitHubClient = new GitHubClient(new ProductHeaderValue("KavitaUpdate"));
        ...
        protected override async Task<bool> DoFetchReleasesAsync()
        {
            ...
            var releases = (await _gitHubClient.Repository.Release.GetAll("Kareadita", RepoName)).ToArray();
            var validReleases = releases
                .Where(r =>  r.TagName.StartsWith("v") &&  VersionUtil.IsValid(r.TagName.Substring(1))) // TODO: Add this back in
                .Take(3)
                .Reverse();
            ...
            foreach (var release in validReleases)
            {
                ...
                var branch = release.Assets.Any(a => a.Name.StartsWith($"{RepoName}.main")) ? Branch.Master : Branch.Develop;
        ...
        private async Task<bool> ProcessRelease(Octokit.Release release, Branch branch, string version)
        {
            ...
            var changes = ReleaseChangesGroup.Matches(releaseBody);
            if (changes.Any())
            {
                updateEntity.Changed.Clear();

                foreach (Match match in changes)
                {
                    updateEntity.Changed.Add(match.Groups["text"].Value);
                }
            }
